...
// DFlash2 drafts a whole block at once: one anchor token followed by seven
// masks, all five layers attending bidirectionally inside a 2048-token
// sliding window.  Two grouped dynamic causal convolutions per layer and a
// codebook candidate selector separate it from the DFlash v1 drafter.
struct DFlashConfig {
    ...
    // DFlash2 additions over v1.
    static constexpr bool has_convolution      = true;
    static constexpr bool has_candidate_select = true;
    static constexpr int block_size            = 8;
    static constexpr int conv_kernel_size      = 2;
    static constexpr int conv_group_size       = 16;
    static constexpr int conv_groups           = hidden / conv_group_size;
    static constexpr int conv_projection_rows  = 2 * conv_kernel_size * conv_groups;
    static constexpr int selector_rank         = 256;
    static constexpr int selector_top_k        = 16;
    static constexpr int vocabulary            = TextConfig::output_rows;
};
...
inline constexpr std::uint32_t kMaximumDFlashDraftTokens = DFlashConfig::block_size - 1;
