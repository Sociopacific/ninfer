...
PagedKVCacheLayout plan_cache(LayoutBuilder& builder, std::uint32_t layers, std::uint32_t capacity,
    ...
    const bool packed_int4      = dtype == DType::U8;
    const bool quantized        = dtype == DType::I8 || packed_int4;
    const std::int32_t expected = packed_int4 ? kKvQuantGroupInt4 : kKvQuantGroup;
    if ((!quantized && (dtype != DType::BF16 || quant_group != 0)) ||
        (quantized && (quant_group != expected || head_dim % quant_group != 0)) ||
        (packed_int4 && head_dim % 2 != 0)) {
        throw std::invalid_argument("Paged KV cache dtype or quantization is invalid");
    }
    // INT4 packs two codes per byte, so the code plane is half as wide as the head extent.
    const std::int32_t code_extent = packed_int4 ? head_dim / 2 : head_dim;
