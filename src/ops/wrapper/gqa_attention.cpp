...
std::uint32_t validate_cache(const PagedKVLayerView& cache, std::int32_t kv_heads, const char* op) {
    ...
    if (cache.dtype == DType::U8 && cache.quant_group != kQuantGroupI4) {
        throw std::invalid_argument(std::string(op) + ": INT4 KV cache must use quant_group 32");
    }
    ...
    // INT4 keeps two codes per byte, so its code plane is half the head extent wide.
    const std::int32_t code_extent = cache.dtype == DType::U8 ? kHeadDim / 2 : kHeadDim;
