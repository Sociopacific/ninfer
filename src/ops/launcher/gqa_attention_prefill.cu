...
#include "ops/kernel/gqa_attention_prefill_i4.cuh"
...
void gqa_attention_prompt_attention_launch_for(const Tensor& q, const Tensor& positions,
    ...
    static const cudaError_t attr_i4 =
        cudaFuncSetAttribute(gqa_attention_prefill_i4_kernel<Geometry, Metadata>,
                             cudaFuncAttributeMaxDynamicSharedMemorySize, kGqaPrefillI4SmemBytes);
    CUDA_CHECK(attr_i4);

    const auto tokens = static_cast<std::int32_t>(q.ne[2]);
    if (cache.dtype == DType::U8) {
        const dim3 attention_grid(static_cast<unsigned>(div_up(tokens, kGqaPrefillI4Br)),
                                  static_cast<unsigned>(Geometry::QHeads), 1u);
        const Tensor& cache_k_scale = cache.k_scale_pages;
        const Tensor& cache_v_scale = cache.v_scale_pages;
        gqa_attention_prefill_i4_kernel<Geometry, Metadata>
            <<<attention_grid, kGqaPrefillI4Threads, kGqaPrefillI4SmemBytes, stream>>>(
    ...
    if (cache.dtype == DType::U8) {
        ...
        if (tokens >= 128 && Geometry::KVHeads == 2) {
            ...
            const dim3 fill_grid(static_cast<unsigned>(max_tiles),
                                 ...
                                 static_cast<unsigned>(kGqaKvQuantGroupsI4));
            gqa_attention_prefill_fill_i4_page_kernel<Geometry, Metadata>
            ...
            const std::int64_t fill_units =
                static_cast<std::int64_t>(tokens) * Geometry::KVHeads * kGqaKvQuantGroupsI4;
            const int fill_grid =
                static_cast<int>(div_up(fill_units, static_cast<std::int64_t>(kFillWarps)));
            gqa_attention_prefill_fill_i4_kernel<Geometry, Metadata>
