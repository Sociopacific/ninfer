...
// Derived from the int8 kernel; the differences are exactly the storage width.
//
//   * The cache holds two signed 4-bit codes per byte at group 32, so a key row
//     is 128 bytes instead of 256 and carries eight scales instead of four.
//   * K is staged as packed bytes and expanded to int8 in shared memory before
//     the QK MMA, which therefore stays m16n8k32.s8 exactly as at int8. Q is
//     quantized on-chip to int8 per (row, 32-group) so its scale index lines up
//     with K's, leaving one MMA k-step per group instead of two.
//   * V is expanded from nibbles straight to the bf16 tile the PV MMA reads,
//     the same single dequant the int8 kernel performs.
//   * The shared arena is unchanged in size: the int8 V tile the int8 kernel
//     needs is replaced by the two packed staging buffers.
//
// Shared scaffolding lives in gqa_attention_decode.cuh; the swizzled int8 store
// helper is shared with the int8 kernel.
...
__launch_bounds__(WarpsPerCta * 32, MinBlocksPerSm) __global__
    void gqa_attention_decode_i4_tiled_kernel(
        const __nv_bfloat16* q, CacheInput input, const std::int32_t* pos, std::uint8_t* cache_k_i4,
        std::uint8_t* cache_v_i4, __half* cache_k_scale, __half* cache_v_scale,
    ...
    constexpr int Groups               = kGqaKvQuantGroupsI4;
    constexpr int GroupKc              = kGqaKvQuantGroupI4 / 32;
    constexpr int DPacked              = D / 2;
    ...
    // 32-dimension group at a time instead of carrying all eight fragments in
    // registers across the whole kernel. The main arena holds the expanded K
    // int8 tile, the V bf16 tile, and the two packed staging buffers.
    ...
    __nv_bfloat16* v_bf16 = reinterpret_cast<__nv_bfloat16*>(r_s + Bc * D);
    std::uint8_t* k_i4    = reinterpret_cast<std::uint8_t*>(r_s + 3 * Bc * D);
    std::uint8_t* v_i4    = k_i4 + Bc * DPacked;
    ...
    if constexpr (CacheInput::writes_cache) {
        ...
        // One warp per (token, 32-group): every lane owns exactly one dimension,
            // so the warp reduction is the group absmax and adjacent lanes pair
            // into one byte.
        for (int pair = warp; pair < valid_tokens * Groups; pair += Wc) {
            ...
            float kamax             = warp_max(fabsf(kv0), FullMask);
            float vamax             = warp_max(fabsf(vv0), FullMask);
            const __half ksh        = __float2half_rn(
                kamax > 0.0f ? kamax / static_cast<float>(kGqaKvQuantMaxI4) : 0.0f);
            const __half vsh = __float2half_rn(
                vamax > 0.0f ? vamax / static_cast<float>(kGqaKvQuantMaxI4) : 0.0f);
            ...
            const int k_code        = gqa_kv_quant_code_i4(kv0, k_inv);
            const int v_code        = gqa_kv_quant_code_i4(vv0, v_inv);
            ...
            if ((lane & 1) == 0) {
                ...
                cache_k_i4[co] = gqa_kv_pack_i4x2(k_code, k_odd);
                cache_v_i4[co] = gqa_kv_pack_i4x2(v_code, v_odd);
            }
            if (lane == 0) {
                const std::int64_t so =
                    gqa_kv_quant_scale_index_i4<Geometry>(physical_page, kv_head, grp, page_offset);
    ...
    for (int unit = warp; unit < RowCount * Groups; unit += Wc) {
        ...
        const float amax = warp_max(fabsf(x0), FullMask);
    ...
    auto issue_kv_tile = [&](int tile_k0, int physical_page) {
        for (int key_l = tid; key_l < Bc; key_l += Threads) {
            ...
            if (key >= split_start && key < split_end) {
                const std::int64_t off = gqa_kv_quant_scale_index_i4<Geometry>(
        ...
        // Packed codes are staged linearly; K is expanded into its swizzled int8
        // tile at the top of the key loop, V straight to bf16 by the loader warps.
#pragma unroll 1
        for (int chunk = tid; chunk < Bc * (DPacked / 16); chunk += Threads) {
            ...
            if (key >= split_start && key < split_end) {
                ...
                ninfer::ops::cp_async<16>(&k_i4[key_l * DPacked + (d >> 1)], &cache_k_i4[off]);
                ninfer::ops::cp_async<16>(&v_i4[key_l * DPacked + (d >> 1)], &cache_v_i4[off]);
            } else {
                store_vec(&k_i4[key_l * DPacked + (d >> 1)], make_int4(0, 0, 0, 0));
                store_vec(&v_i4[key_l * DPacked + (d >> 1)], make_int4(0, 0, 0, 0));
            }
        }
    ...
    for (int kb = 0; kb < key_blocks; ++kb) {
        ...
        // Expand the staged nibbles into the int8 tile the QK MMA reads. This
        // frees the packed K buffer for the next prefetch issued further down.
        for (int chunk = tid; chunk < Bc * (D / 16); chunk += Threads) {
            ...
            store_vec(&k_i8[key_l * D + gqa_small_t_tc_swz(key_l, dc * 8) * 2],
...
#pragma unroll 1
            for (int chunk = loader_tid; chunk < Bc * (D / 8); chunk += VLoaderThreads) {
                ...
                if (key >= split_start && key < split_end) {
                    const int grp = d >> 5;
                    float vs      = 0.0f;
                    if ((lane & 3) == 0) { vs = __half2float(v_scale_s[key_l * Groups + grp]); }
                    vs = __shfl_sync(FullMask, vs, grp * 4);
                    store_vec(dst,
                              gqa_kv_dequant_i4x8_from(&v_i4[key_l * DPacked + (d >> 1)], vs));
                } else {
